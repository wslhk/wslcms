		  ...
		  <div class="int_info">
			  <!--case1 选择平台-->
				...
				<div style="" class="create_ad_target content_table">
				  <h3 class="m_subtit account_title">专题编辑</h3>
                  <form id="form_adapter" action="special_do.action" method="post" enctype="multipart/form-data">
<input type="hidden" name="action" value="edit" />
<input type="hidden" name="referer" value="${referer}" />


<input value="${special.id}"  name="id" type="hidden" />
					<table class="form-table">
						<tbody>
                    
                        
<tr><th><label for="role">名称：</label></th>
  <td><input type="text" name="title" class="ipt ipt_text" value="${special.title}" /><span></span>
</td></tr>

<tr>
  <th>platformId</th>
  <td><input type="text" name="platformId" value="${special.platformId}" /></td>
</tr>

<tr>
  <th>发布 1发布 0 不发布</th>
  <td><input type="text" name="publish" value="${special.publish}" /></td>
</tr>
<tr><th><label for="role">icon：</label></th>
<td><img width="50" src="../uploadimg/special/${special.icon}"/>
<input type="hidden" name="icon" value="${special.icon}"/>
<input type="file" name="file_icon"  class="ipt ipt_text" />
