...
$().ready(function(){
	...
	if(picture!=''){
		...
		currentImg.src = 'http://www.ifenghui.cn/data/creations/${vpage.picture}';
...
<li>当前第 ${vpage.position} 页</li>
    <li><a href="vcomic_frame.action?pageId=${vpageNext.id}&level=4">下一页(${vpageNext.position})</a></li>
    </c:if>
</ul>

<form id="posts-filter" action="vcomicframe_do.action" method="post">
<!--
<p class="search-box">
...
<table class="wp-list-table widefat  media" cellspacing="0">
	...
	<tr>
		...
		<th>类型</th>
		<th>gif图片</th>
		<th>背景颜色</th><th>背景声音</th>
		<th>音效</th>
		<th>特效</th>
		<!--<th><span class="manage-column num sortable desc">切换效果</span></th>-->
		<th>片头视频</th>
			</tr>
	</tfoot>
<tbody id="the-list">
    ...
    <c:forEach items="${listframeView}" var="frame">
		...
		<th><input type="hidden" name="viewId" value="${frame.viewId}" />
		...
		<td width="200">				
        <c:if test="${frame.level==4}">缩略图
        <span class="viewicon" style="width:200px;height:200px;" x='${frame.view.x}' y='${frame.view.y}' w='${frame.view.w}' h='${frame.view.h}'>
			<img src="http://www.ifenghui.cn/data/creations/${vpage.picture}" class="imgView"/>
			</span>
             
        </c:if>
        
        <c:if test="${frame.level==3}">
        <span style="width:80px;height:80px;">
			<img src="../t2/detail/150/250/1/${frame.page.picture}" class="imgView"/>
			</span>
        </c:if>
       <c:if test="${frame.level==2}">
       ${frame.chapter.name}
        </c:if>
        <c:if test="${frame.level==1}">
       ${frame.comic.name}
        </c:if>
		 </td>
		<td><input type="hidden" name="level" value="${frame.level}"  />
<c:if test="${frame.level==1}">整本</c:if>
<c:if test="${frame.level==2}">章节</c:if>
<c:if test="${frame.level==3}">单页</c:if>
<c:if test="${frame.level==4}">分镜头</c:if>
</td>
		<td><c:if test="${frame.view.isGif==true}">有</c:if><c:if test="${frame.view.isGif==false}">无</c:if>
        <a href="vcomic_view_gif.action?id=${frame.view.id}">编辑gif</a>
        </td>
		<td><select name="bgColor">
        <option value="" >无颜色</option>
        	<option value="1"  <c:if test="${frame.bgColor==1}"> selected="selected" </c:if>>白色</option>
            <option value="0"  <c:if test="${frame.bgColor==0}"> selected="selected" </c:if>>黑色</option>
        </select></td>
		...
		<td>
        <c:if test="${frame.level==4||frame.level==3}">
        <span class="mediaSelect">${frame.mediaSound.title}</span>
        </c:if>
        <input name="soundId"  type="hidden"  value="${frame.soundId}"></td>
		<td>
         <c:if test="${frame.level==3||frame.level==4}">
        <span class="effectSelect">${frame.effect.title}</span>
        </c:if>
        <input name="effectId"  type="hidden" value="${frame.effectId}"></td>
		<!--<td >
        <c:if test="${frame.level!=4}">
        <span class="switcherSelect">${frame.switcher.title}</span>
        </c:if>
        <input name="switcherId"  type="hidden" value="${frame.switcherId}"></td>-->
		<td >
        <c:if test="${frame.level==4||frame.level==3}">
