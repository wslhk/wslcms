...
<h2>单页
<a href="page_new.action?chapterId=${param.chapterId}" class="button add-new-h2">添加</a>
<a href="chapter.action?chapterId=${page.chapterId}" class="button add-new-h2">返回</a>
</h2>
...
<c:forEach items="${listpage}" var="page">
	<tr id='user-1' class="alternate"><th scope='row' class='check-column'><input type='checkbox' name='ids[]' class='ckb' value='${ad.id}' /></th>
	  <td class="username column-username">${page.id}</td>
	<td class="username column-username"><strong><a href="ad_edit.action?id=${ad.id}&type=${ad.type}"><img src="../uploadimg/comic/${page.picture}" width="80" /></a></strong><br />
	...
	<td class="role column-role"><a href="page_do.action?action=up&id=${page.id}">上移</a>,<a href="page_do.action?action=down&id=${page.id}">下移</a></td>
	</tr>	
      </c:forEach>
      </tbody>
</table>
