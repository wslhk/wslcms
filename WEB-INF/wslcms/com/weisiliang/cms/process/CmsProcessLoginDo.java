...
public class CmsProcessLoginDo extends CmsProcessBase implements CmsProcess{
	...
	private boolean authLogin(HttpServletRequest request,String username,String password){
		if(ConfigWSL.getMessage("auth")!=null&&ConfigWSL.getMessage("auth").equals("1")){
		
			
			if(ConfigWSL.getMessage("auth_username")!=null
					&&ConfigWSL.getMessage("auth_username").equals(username)
				&&ConfigWSL.getMessage("auth_password")!=null
				&&ConfigWSL.getMessage("auth_password").equals(password)
					
					){
				request.getSession().setAttribute(CmsFactoryImpl.wslAuthKey,"1");
				return true;
			}else{
				request.getSession().removeAttribute(CmsFactoryImpl.wslAuthKey);
				return false;
			}
